...
void HttpRequestHandler::handleRequest(Transport *transport) {
  ...
  bool cachableDynamicContent =
    (MatchAnyPattern(path, RuntimeOption::StaticFileGeneratorsInclude) &&
     !MatchAnyPattern(path, RuntimeOption::StaticFileGeneratorsExclude));
  ...
  if (cachableDynamicContent) {
    ...
    if (DynamicContentCache::TheCache.find(key, data, len, compressed, mtime)) {
      if (RuntimeOption::StaticFileLifeTime == 0 || 
          RuntimeOption::StaticFileLifeTime > time(NULL) - mtime) {
        sendStaticContent(transport, data, len, mtime, compressed, path);
        ServerStats::LogPage(path, 200);
        return;
      } else DynamicContentCache::TheCache.revoke(key); 
    }
  }
