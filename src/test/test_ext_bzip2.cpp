...
bool TestExtBzip2::test_bzerrno() {
  Variant f = f_fopen("test_ext_bzip2.tmp", "w");
  f_fwrite(f, "this is a test");
  f_fclose(f);
  f = f_bzopen("test_ext_bzip2.tmp", "r");
  f_bzread(f);
  int ret = f_bzerrno(f);
  f_bzclose(f);
  VS(ret, -5);
...
bool TestExtBzip2::test_bzwrite() {
  ...
  Variant bz = f_bzopen("test_ext_bzip2.tmp", "w");
  VERIFY(bz);
  VS(f_bzwrite(bz, str), 10);
  f_bzflush(bz);
  VERIFY(f_bzclose(bz));

  bz = f_bzopen("test_ext_bzip2.tmp", "r");
